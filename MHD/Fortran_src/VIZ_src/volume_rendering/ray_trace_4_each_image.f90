...
!
     ...
     &    ie_surf, isurf_end, xi, field_pvr%d_pvr, r_org(1) )
      call cal_field_on_surf_vector(numnod, numsurf, nnod_4_surf,       &
     &    ie_surf, isurf_end, xi, field_pvr%v_nod, vec_org)
...
!   find 3D coordinate of exit point on exit surface
     ...
     &    ie_surf, isurf_end, xi, field_pvr%d_pvr, r_tgt(1) )
        call cal_field_on_surf_vector(numnod, numsurf, nnod_4_surf,       &
     &    ie_surf, isurf_end, xi, field_pvr%v_nod, vec_tgt)
...
!write(50+my_rank, *) "ray_len: ", ray_len
          if(iflag_fixsize .eq. 1) then
            ray_len_left = ray_left + ray_len
!write(50+my_rank, *) "ray_len_left: ", ray_len_left
!write(50+my_rank, *) "xx_st: ", xx_st
!write(50+my_rank, *) "xx_tgt: ", xx_tgt
            xx_lic_last = xx_st
            step_cnt = 1
            do while(ray_len_left .ge. step_size)
              ray_len_left = ray_len_left - step_size
!write(50+my_rank, *) "step_size", step_size
!write(50+my_rank, *) "step_idx", step_cnt, "ray_len_left: ", ray_len_left, "ray_left", ray_left
              ratio = (step_size*step_cnt - ray_left) / ray_len
!write(50+my_rank, *) "ratio: ", ratio
              ...
              &          k_size, k_ary, field_pvr%v_nod, xx_lic, isurf_end,             &
...
!write(50+my_rank, *) "-xx_lic_start ", xx_lic_last, " -xx_lic_end ", xx_lic
!write(50+my_rank, *) "lic_V: ", c_tgt(1)
              call s_set_rgba_4_each_pixel(viewpoint_vec, xx_lic_last, xx_lic,    &
